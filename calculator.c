...
Token readPeekToken(Expr *expr, int is_peek) {
    ...
    while ( 1 ) {
        ...
        if ( ch == '\0' && resultToken.token == -1 ) {
            ...
            return resultToken;
        } else if ( ch == '+' && resultToken.token == -1 ) {
            ...
            return resultToken;
        } else if ( expr->str[pos-1] == ':' && expr->str[pos] == '=' ) {
            expr->pos += 1;
...
double parseAtom(Expr *expr, Variables *var) {
    ...
    if ( resultToken.token == OPEN_BRACKET1 ||  resultToken.token == OPEN_BRACKET2 ||
        ...
        } else if ( condition.token == MORE_TOKEN ) {
            if ( cond1 > cond2 ) return res1;
            else return res2;
        } else {
            if ( cond1 == cond2 ) return res1;
    ...
    } else if (resultToken.token == EPSILON_TOKEN ) {
        return EPSILON;
    } else if (resultToken.token == M_PI_TOKEN ) {
        return M_PI;
    } else if (resultToken.token == M_E_TOKEN ) {
        return M_E;
    } else if (resultToken.token == MINUS_TOKEN ) {
        resultToken = readPeekToken(expr, 0);
        double res;
        if ( resultToken.token == OPEN_BRACKET1 || resultToken.token == OPEN_BRACKET2 ||
             resultToken.token == OPEN_BRACKET3 ) {
        ...
        return -1 * res;
    } else if (resultToken.token == NUMBER_TOKEN){
        return resultToken.value;
    } else if (resultToken.token == VARIABLE_TOKEN){
